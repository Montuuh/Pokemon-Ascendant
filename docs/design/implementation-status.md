...
>
> Update a row whenever a system's status changes. Last reviewed 2026-09-25 (v0.7.7).
...
| Field effects | §4.3 | — | — | ☐ v0.8.4, after multi-enemy (v0.8.1) |
| Cities | §2.1.4, §2.11 | `run/cities.ts`, `run/run.ts` (`arriveAtCity`, `enter-building`, `depart-city`, `sell-item`), `run/economy.ts` (the City shelf), `ui/screens/city/*` | `cities`, `city`, `safari`, `blackMarket`, `run`, `runBalance` (full run), `ring` (balance), `balance/market`, `e2e/city`, `e2e/safari`, `e2e/black-market`, `e2e/economy`, `e2e/progression`, `e2e/run` | ◐ Gym → City → next Region. **Pallet Town** and **Celadon City** drawn as lobbies (generated pixel art, doors as boxes over the buildings in `towns.ts`; v0.7.7 redrew the square's court as the Challenge Ring and the plaza's as the Pokémon Coliseum by pasting only the changed patch into the original, palette-snapped, and removed the stairs beside the Game Corner); open doors: Center (heal + Therapy), the Mart (8 + Poké Balls, sells held items at 30 %) / Celadon's **Department Store** (five floors as tabs, 21 slots, ×1.3, a re-roll restocks one floor — `rollShopStock(…, 'department-store')`), the Dojo (+30 % and every reached stage's tutor list in Celadon — `tutorListFor`), the **Game Corner** in Celadon (`CASINO`, `spin-wheel`/`pull-slots`, own `CasinoRNG` stream, `ui/screens/GameCornerScreen.tsx`); the **Ring** as a building in both (§2.9.4.1, v0.7.7: `CityBuilding` `ring`, `CITIES[id].ringName`, `RING`, `rollRing`/`resolveRing`, `enter-ring`/`leave-ring` in `run.ts`, `buildRingScenario`, `ui/screens/RingScreen.tsx` + `RingPrizeScreen.tsx` + `RingGuide.tsx` (the How to play); `closeRing` heals the Box whenever the ladder ends; per-City offsets tuned by `playRing`); the committing doors' warning (§2.11.0, `ui/components/ConfirmLeave.tsx`); **Team Rocket's Black Market** in Celadon (§2.11.6, v0.7.7: `run/blackMarket.ts` — `BLACK_MARKET`, `rollBlackMarket`, `wagerChance`, `SHOWCASE_CAP` — the `push-switch`/`enter-black-market`/`market-*` actions in `run.ts`, its own `MarketRNG` stream, run save v13; `ui/screens/GameCornerRoom.tsx` (the whole FRLG room as the Game Corner: the slot banks and roulette tables open the machines' panels in `GameCornerScreen.tsx`; the poster, the Grunt, the stairs, the locked hatch; the table and the hatch by `npm run art:props`) and `BlackMarketScreen.tsx`, art by `npm run art:rocket`; the harness's `takeMarket` is off by default); the gate is the Reflection; the **Safari Zone** in both (§2.11.6, v0.7.6: `run/safari.ts` — `SAFARI`, `rollSafari`, `rollHunt`, `planOf`/`afterTurn`/`throwOdds` — the `safari-*` actions in `run.ts`, its own `SafariRNG` stream, run save v12; `ui/screens/safari/` — the screen, the How to play guide (`SafariGuide`), the FRLG tiles (`tiles.ts`, `CellArt`, `npm run art:safari`); tests `safari` (rules), `balance/safari` (the bands, with `balance/autoSafari.ts` as the stalker), `e2e/safari`). The Dojo's extra-moves counter opens an "in development" panel. **Not yet:** §2.11.1's Daycare and PC Box services (the map's Box panel stands in), §2.11.2.1's scored curation (every shelf is the slot table, drawn at random). The Trauma Salve Cache Hub upgrade is sold and stocks the first City |
...
| Enemy AI and intents | §5.1–§5.7 | `intents.ts`, `slots.ts` | `intents`, goldens | ✅ · the intent queue (§5.5.1, v0.7.5): `EnemyCombatant.next`, planned in `declareIntent` under Trainer's Instinct (`relicsQueueIntents`), kept while `scoreIntent` > 0; the "Next" chip in `EnemyPanel`. Battle Tracker scouts in `encounter.ts` (`applyPerks`) |
...
| XP, levels, evolution | §6.2, §6.3 | `run/xp.ts`, `ui/screens/EvolutionScreen.tsx` | `run`, `runBalance`, `e2e/progression` | ✅ 63 branches, archetype picked per evolution. **Evolution Items** (§6.3.2, v0.7.5): `content/data/evolution-items.json`, `use-stone` in `run.ts` (`stoneUse`, `stonesForBox` in `xp.ts`), the Move Manager's stones, City shop slots, the Mysterious Stone event and Eevee's Stone Cache; run save v11 |
| Moves, abilities, the Dojo | §6.4–§6.7 | `abilities.ts` (19 hooks), `run/xp.ts`, `ui/components/MoveManager.tsx`, `ui/screens/DojoScreen.tsx` | `abilities`, `moveEffects`, `run`, `e2e/progression` | ✅ pool, Move Manager, 3 TMs, Dojo at canon prices; every line's hidden ability authored (§6.8.3, v0.7.5); 3 abilities wait on field effects (v0.8.4) |
...
| 6 | Four lines' first pool entry is their most characteristic passive (§6.5.1) | It is; for Oddish, Magikarp and Psyduck that passive (Chlorophyll, Swift Swim, Cloud Nine) reads the field system, and says so | **v0.8.4** |
...
| Relics | 25 Common + 18 Uncommon + 11 Rare + 10 Legendary | 64 · all live since v0.7.5; every row carries its meta tier and, for Tier 2, its discovery criterion |
