...
| Game glyphs: type ×15, status ×6, intent ×7, trait ×5, modifier ×2, rarity ×5, medal ×4, action ×10, nav ×8, toolbar ×6 | Authored SVG set (`docs/art/icon-set.md`). The **type** glyphs are the set's own 32×32 filled silhouettes in the canonical type hues — the Gen I fifteen, no more. *(2026-09-22: the franchise-emblem vectors that had replaced them in Unity's CL-023 were swapped back out; the user's call — the simple silhouettes sit with the pixel-art sprites, the emblems fought them, and the authored set carries no third-party licence.)* | ✅ 71 (`public/art/icons/<family>`) |
...
| Main-menu vista, Region map plate | 16:9 | `gemini-3-pro-image` | 2 |
| Consumable item icons | 1:1 | **real** — Serebii S&V renders | 11 |
| TM discs, one per type | 1:1 | **real** — S&V bag sprites, Bulbagarden | 4 |
| Map node markers | 1:1 | **real** — badge / building / tile / trainer sprites on a shared disc | 8 |
