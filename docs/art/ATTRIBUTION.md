...
| Team Rocket's Black Market (v0.7.7): the Celadon Game Corner's back wall (the prize counter, the posters, the Grunt; before and after the switch, the Grunt's tiles taken from the Archives' empty copy of the same map), and the Rocket Hideout B1F's wall, floor tile and stairs | **The real assets**, cut on the 16-px grid from the FireRed / LeafGreen maps via the [Bulbagarden Archives](https://archives.bulbagarden.net) (`npm run art:rocket`) | © Nintendo / Creatures / GAME FREAK; fan use | `public/art/game-corner`, `public/art/black-market` |
