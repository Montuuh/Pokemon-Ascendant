...
- **Unresolved points** are written as `> ⚠️ **OPEN (YYYY-MM-DD)**: …` inline in the section they affect, naming who decides. Never fold a guess into canon, and never start a parallel list of them.
- **Design changes follow the `design-change` skill**: read the §, generate 2–4 named options with pillar impact, present without picking, the user decides, then record it in the section itself (plus the codex date, and the roadmap if scope moved).
