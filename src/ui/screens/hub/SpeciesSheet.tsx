...
export function SpeciesSheet({ speciesId, account, initialTab = 'record', onSpecies }: {
  ...
  const met = entry.encounters > 0 || entry.defeats > 0 || entry.recruits > 0 || entry.winsWith > 0;
  ...
  return (
    <div data-testid="dex-sheet" data-species={speciesId} data-met={met}>
      <div className={styles.hero} style={{ ['--sheet-type' as string]: `var(--type-${s.types[0]})` }}>
        ...
        <span className={styles.heroArt}>
          {spriteOk ? (
            ...
            <img src={spriteOf({ speciesId }, 'front', rank >= 2)} alt={s.name} className={`${styles.heroSprite} ${met ? '' : styles.heroSilhouette}`} onError={() => setSpriteOk(false)} />
          ) : (
            <img src={portraitOf({ speciesId })} alt={s.name} className={`${styles.heroSprite} ${met ? '' : styles.heroSilhouette}`} />
          )}
        </span>
